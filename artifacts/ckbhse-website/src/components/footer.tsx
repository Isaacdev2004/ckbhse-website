      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8 lg:gap-12 pb-12 border-b border-secondary-foreground/10">
          ...
          <div className="lg:col-span-2">
            <Link href="/" className="flex items-center gap-2 group mb-6" data-testid="link-footer-logo">
              <div className="w-10 h-10 bg-primary rounded-lg flex items-center justify-center">
                <Shield className="w-6 h-6 text-primary-foreground" />
              </div>
              <div className="flex flex-col">
                <span className="font-display font-bold text-lg leading-none">CKBHSE</span>
                <span className="text-xs text-secondary-foreground/70 font-medium">Limited</span>
              </div>
            </Link>
            <p className="text-sm text-secondary-foreground/80 mb-6 max-w-sm">
              The HSEQ consultancy for organisations that take safety seriously. Expert consulting, accredited training, and modern compliance solutions.
            </p>
            <div className="space-y-3">
              <div className="flex items-center gap-2 text-sm">
                <Mail className="w-4 h-4 text-primary" />
                <a href="mailto:info@ckbhse.co.uk" className="hover:text-primary transition-colors" data-testid="link-email">
                  info@ckbhse.co.uk
                </a>
              </div>
              <div className="flex items-center gap-2 text-sm">
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-secondary-foreground/80">London, United Kingdom</span>
              </div>
            </div>
          </div>
        ...
        <div className="pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex flex-wrap items-center gap-4 text-xs text-secondary-foreground/60">
            <p>&copy; {new Date().getFullYear()} CKBHSE Limited. All rights reserved.</p>
            {legalLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
